...
QUERY;
    ...
    public function testQueryRuleBasedRelatedProducts()
    {
        $promoAttr = $this->fixtures->get('attr');
        $attributeCode = $promoAttr->getAttributeCode();

        $productAction = $this->objectManager->create(ProductAction::class);
        $productAction->updateAttributes(
            [
                $this->fixtures->get('product1')->getId(),
                $this->fixtures->get('product2')->getId(),
                $this->fixtures->get('product3')->getId(),
            ],
            [$attributeCode => $promoAttr->getData('option_1')],
            0
        );

        $productSku = $this->fixtures->get('product1')->getSku();
        $query = <<<QUERY
{
    products(filter: {sku: {eq: "{$productSku}"}})
    {
